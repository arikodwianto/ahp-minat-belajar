            ...
            <div class="card shadow-lg border-0 rounded-3">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('perbandingan_alternatif.store', $siswa->id) }}" method="POST">
                        ...
                        <h5 class="mb-3">Perbandingan Alternatif</h5>
    ...
    @foreach($alternatifs as $j => $alt2)
        @if($i < $j)
            ...
            <tr>
                ...
                <td>{{ $alt2->nama }}</td>
                <td>
                    <select name="alternatif_{{ $alt1->id }}_vs_{{ $alt2->id }}" class="form-select" required>
                        @for($n = 1; $n <= 9; $n++)
                            <option value="{{ $n }}" {{ (isset($existing['nilai']) && $existing['nilai'] == $n) ? 'selected' : ($n==1 ? 'selected' : '') }}>
                                {{ $n }} -
                                @switch($n)
                ...
                <td>
                    <select name="pilihan_{{ $alt1->id }}_vs_{{ $alt2->id }}" class="form-select" required>
                ...
                <td>
                    <input type="text" class="form-control" name="alasan_{{ $alt1->id }}_vs_{{ $alt2->id }}" placeholder="Masukkan alasan" value="{{ $existing['alasan'] ?? '' }}">
                </td>
            </tr>
        @endif
    @endforeach
